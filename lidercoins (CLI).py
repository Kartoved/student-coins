...
def add_student():
    ...
    while True:
        student_coins = input(
            "Напишите имя ученика и изначальное количество баллов: ").split()
        ...
        with open(my_path + student_coins[0].title() + '.txt', "a", encoding='utf-8') as f:
            f.write(
                f"{now} Добавлен(а) {student_coins[0].title()} с {student_coins[1]} лидеркоином(ами). ")
...
def delete_student():
    ...
    choose_group()
    student_name = input(
        "Напишите имя ученика, которого хотите удалить: ").title()
...
def change_coins():
    ...
    choose_group()
    while 1:
        student_change_coins = input(
            "Кому, сколько и за что лидеркоинов? ").split()
        ...
        if student_change_coins[0].title() not in group_dic:
            question = input(
                "Такого ученика нет в списке! Желаете его добавить? да/нет ")
            if question.lower() == 'да':
                add_student()
            else:
                continue
        else:
            group_dic[student_change_coins[0]] = group_dic[student_change_coins[0]
                                                           ] + int(student_change_coins[1])
            with open(group + '.json', 'w', encoding='utf-8') as f:  # перезаписывает словарь в файл
                json.dump(group_dic, f, ensure_ascii=False, sort_keys=True)
            with open(my_path + student_change_coins[0] + '.txt', "a", encoding='utf-8') as f:
                ...
                f.write(
                    f" Сейчас у него(ее) {group_dic[student_change_coins[0]]}")
            with open(my_path + group + '.txt', 'w', encoding='utf-8') as f:
                f.write(str(group_dic))
            print(
                f"\n{now} {student_change_coins[0].title()} зачислено {student_change_coins[1]} лидеркоин(ов\а) за {' '.join(student_change_coins[2:])}")
...
def student_coins():
    ...
    while 1:
        ...
        if name_of_student.lower() == 'выйти':
            ...
            print(
                f"{name_of_student} имеет {group_dic[name_of_student]} лидеркоинов")
...
def change_all_students():
    ...
    # TODO сделать так, чтобы на выйти выходил из программы
